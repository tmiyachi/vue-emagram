...
for tt in theta:
    ...
    dryline.append(
        [[round(t.magnitude, 2), int(p.magnitude)] for p, t in zip(pressure, result)]
    )
...
for w in mixingratio:
    ...
    mixingratioline.append(
        [[round(t.magnitude, 2), int(p.magnitude)] for p, t in zip(pres, td)]
    )
